...
__name(parse_hysteria, "parse_hysteria");
...
__name(parse_vle55, "parse_vle55");
...
__name(parse_vme55, "parse_vme55");
...
__name(parse_shadowsocks, "parse_shadowsocks");
...
__name(parse_tr0jan, "parse_tr0jan");
...
__name(parser_mieru, "parser_mieru");
...
__name(parse_juicity, "parse_juicity");
...
__name(fetchWebPageContent, "fetchWebPageContent");
...
async function fetchAndProcessUrl(url) {
  ...
  if (outbounds === null && jsonObject) {
    ...
    if (is_mieru) {
      ...
      let protocol2 = findFieldValue(jsonObject, "protocol");
      let mieruThrone = `mierus://${userName}:${userPassword}@${ipAddress}?profile=${profile}&port=${port}&protocol=${protocol2}&multiplexing=${multiplexing}#${userName}`;
      let mieruNyameBox = `mierus://${userName}:${userPassword}@${ipAddress}:${port}?transport=${protocol2}&multiplexing=${multiplexing}#${userName}`;
      let link = whoNekoBox.toLowerCase() === "nyamebox" ? mieruNyameBox : mieruThrone;
      return link;
    }
    ...
    if (is_juicity) {
      let uuid = findFieldValue(jsonObject, "uuid");
      let password = encodeURIComponent(findFieldValue(jsonObject, "password"));
      let server2 = findFieldValue(jsonObject, "server");
      let param = new URLSearchParams({
        security: "tls",
        sni: findFieldValue(jsonObject, "sni"),
        allowInsecure: findFieldValue(jsonObject, "allow_insecure")
      });
      let juicityLink = `juicity://${uuid}:${password}@${server2}?congestion_control=${findFieldValue(jsonObject, "congestion_control")}&${param}#${server2}`;
      return juicityLink;
    }
...
__name(fetchAndProcessUrl, "fetchAndProcessUrl");
...
function isValidBase64(str2) {
  ...
  if (!base64Regex.test(str2)) return false;
  if (str2.length % 4 !== 0) return false;
  try {
...
__name(v2rayLinksHandle, "v2rayLinksHandle");
var targetUrls = [
...
async function processUrls(targetUrls2) {
  ...
  const asyncPool = /* @__PURE__ */ __name(async (poolLimit, array, iteratorFn) => {
    const results2 = [];
    const executing = [];
...
__name(processUrls, "processUrls");
var worker_default = {
